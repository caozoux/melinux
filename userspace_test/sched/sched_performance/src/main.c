...
static unsigned long cpu_1_ms_count;
static int args_sched_policy = 0;
static int args_sched_priority = -1;
static int args_util_increase = 0;
static int args_report = 0;
static enum test_case args_test_case = CASE_NULL;

static unsigned long *report_data;
...
static void cpu_1_ms_time(void)
{
	unsigned long cnt = cpu_1_ms_count;
	while(cnt--);
}
...
void *threadFunc(void *param)
{
	...
	int i, j;
	int cpu_util = data->cpu_util;
	int sleep_ms = 1000 - arg_cpu_util * 10;

	while(1) {
		j = 0;
		for (i = 0; i < cpu_util; ++i) {
			for (j = 0; j < 10; j++) {
				cpu_1_ms_time();
				report_data[data->id] += 1;
			}
		}
...
void test_case_balance()
{
	...
	for (i = 0; i < cnt; ++i) {
		...
		thread_data_array[i].cpu_util= 0;
...
static unsigned long old_data = 0;
...
static void caculte_cpu_10_counts(void)
{
	...
	cpu_1_ms_count = cpu_10_ms_count/10;
}
...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		static struct option long_options[] =
		{
			...
			{"util_increase", required_argument,	0,	'e'},
			{0,0,0,0}
		};
	
		int option_index = 0;
		choice = getopt_long( argc, argv, "vht:u:p:c:re:", long_options, &option_index);
		...
		switch( choice )
		{
			...
			case 'u':
				...
				if (arg_cpu_util > 100) {
					fprintf(stderr, " cpu_util must be litter 100\n");
					return -1;
				}
				break;
			...
			case 'e':
				args_util_increase = atoi(optarg);
				break;
	...
	if (args_thread_cnt) {
		report_data = malloc(sizeof(unsigned long) *args_thread_cnt);
		for (i = 0; i < args_thread_cnt; ++i) {
			thread_data_array[i].id = i;
			thread_data_array[i].cpu_util = arg_cpu_util + i*args_util_increase;
			pthread_create(&thread_array[i], NULL, threadFunc, &thread_data_array[i]);
		}
		if (args_report)
			alarm(1);
