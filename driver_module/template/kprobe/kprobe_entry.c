...
static int kretprobe_handler(struct kretprobe_instance *p, struct pt_regs *regs);
static int kretprobe_post_handler(struct kretprobe_instance  *p, struct pt_regs *regs);
...
struct kretprobe kretp1 = {
	.handler    = kretprobe_handler,
	.entry_handler  =  kretprobe_post_handler,
};

static void kprobe_post_handler(struct kprobe *p, struct pt_regs *regs, unsigned long flags)
{
	printk("zz %s pc:%lx \n",__func__, (unsigned long)regs->ip);
}
...
static int __init kprobedriver_init(void)
{
	...
	kp1.symbol_name = sym;
	kretp1.kp.symbol_name = sym;
    ret = register_kprobe(&kp1);
    ret = register_kretprobe(&kretp1);
	return 0;
}
