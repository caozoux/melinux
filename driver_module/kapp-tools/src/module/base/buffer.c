	...
	unsigned int buf_size)
{
	...
	ret = -EINVAL;
	if (buf_size < 10 * KSYS_TRACE_BUF_SIZE)
		return ret;

	memset(buffer, 0, sizeof(struct ksys_variant_buffer));
...
void discard_ksys_variant_buffer(struct ksys_variant_buffer *buffer)
{
	...
	ksys_variant_buffer_mutex_lock(buffer);
	ksys_variant_buffer_spin_lock(buffer, flags);
	buffer->buffers[0].pos = buffer->buffers[1].pos = 0;
	buffer->buffer_toggle = 0;
	ksys_variant_buffer_spin_unlock(buffer, flags);
	ksys_variant_buffer_mutex_unlock(buffer);
}
...
ksys_variant_buffer_reserve(struct ksys_variant_buffer *buffer, size_t len)
{
	...
	real_len += sizeof(struct ksys_variant_buffer_head);
	...
	if (buffer->buffer_toggle == 0) {
		if (buffer->buffers[0].pos + real_len >= buffer->buf_size) {
			...
			buffer->buffers[1].pos = sizeof(struct ksys_variant_buffer_head);
			buffer->buffers[1].head = (void *)buffer->buffers[1].data;
			memset(buffer->buffers[1].head, 0, sizeof(struct ksys_variant_buffer_head));
		} else {
			buffer->buffers[0].head = (void *)buffer->buffers[0].data + buffer->buffers[0].pos;
			memset(buffer->buffers[0].head, 0, sizeof(struct ksys_variant_buffer_head));
			buffer->buffers[0].pos += sizeof(struct ksys_variant_buffer_head);
		}
	} else {
...
ksys_variant_buffer_seal(struct ksys_variant_buffer *buffer)
{
	...
	if ((void *)buffer->buffers[idx].head < (void *)buffer->buffers[idx].data)
		return -EINVAL;
	if ((void *)buffer->buffers[idx].head > (rbound - sizeof(struct ksys_variant_buffer_head)))
		return -EINVAL;

	buffer->buffers[idx].head->magic = KSYS_VARIANT_BUFFER_HEAD_MAGIC_SEALED;
	...
	void __user *ptr_len, void __user *buf, size_t size)
{
	...
	backup_ksys_variant_buffer(variant_buffer);
