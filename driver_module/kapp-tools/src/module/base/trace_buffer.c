	...
	unsigned int buf_size)
{
	...
	memset(buffer, 0, sizeof(struct ksys_trace_buffer));
...
void discard_ksys_trace_buffer(struct ksys_trace_buffer *buffer)
{
	...
	ksys_trace_buffer_mutex_lock(buffer);
	ksys_trace_buffer_spin_lock(buffer, flags);
	buffer->buffer.circle = buffer->buffer.pos = buffer->buffer.tail = 0;
	ksys_trace_buffer_spin_unlock(buffer, flags);
	ksys_trace_buffer_mutex_unlock(buffer);
}
	...
	const void *data, size_t len)
{
	...
	ret = ksys_trace_buffer_write_nolock(buffer, data, len);
...
__ksys_trace_buffer_printk_nolock(struct ksys_trace_buffer *buffer,
	const char *fmt, va_list ap)
{
	unsigned int len = 0;
	unsigned int left;

	len = vsnprintf(buffer->fmt_buffer, KSYS_TRACE_BUF_SIZE, fmt, ap);
	if (len > KSYS_TRACE_BUF_SIZE)
		return len;
	...
	const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	ret = __ksys_trace_buffer_printk_nolock(buffer, fmt, ap);
	va_end(ap);
