...
static void altera_uart_fifo_writel(struct uart_port *port, char chr);
static int altera_uart_uboot_test(void __iomem *address);
/*
...
static void altera_uart_start_tx(struct uart_port *port)
{
	...
	if (port->x_char) {
		...
		altera_uart_fifo_writel(port, port->x_char);
		port->x_char = 0;
		port->icount.tx++;
		return;
	}
	...
	while (xmit->head != xmit->tail) {
		altera_uart_fifo_writel(port, xmit->buf[xmit->tail]);
...
#else
	altera_uart_uboot_test(port->membase);
#endif
}
...
#define CONFIG_SERIAL_ALTERA_UART_MAXPORTS 2
...
static void altera_uart_fifo_writel(struct uart_port *port, char chr)
{
	struct NS16550 *base;
	base=port->membase;
	while ((readb(&base->lsr) & UART_LSR_THRE) == 0)
		 ;
	writeb(chr, &base->thr);
}
...
static int altera_uart_uboot_test(void __iomem *address)
{
    ...
    while (!(readb(&base->lsr) & UART_LSR_TEMT))
        ;

	/* using a dummy loop to avoid it being optimized */
	for (i = 0; i < 2; i++) {
		dummy = readb(&base->msr);
		if (dummy)
			break;
	}

    writel(CONFIG_SYS_NS16550_IER, &base->ier);

    writel(UART_LCR_BKSE | UART_LCRVAL, (ulong)&base->lcr);
    writel(0, &base->dll);
    writel(0, &base->dlm);
    writel(UART_LCRVAL, &base->lcr);
    writel(UART_MCRVAL, &base->mcr);
    writel(UART_FCRVAL, &base->fcr);
    writel(UART_LCR_BKSE | UART_LCRVAL, &base->lcr);
    writel(0x36& 0xff, &base->dll);
    writel((0x36>> 8) & 0xff, &base->dlm);
    writel(UART_LCRVAL, &base->lcr);
#if 1
//--------------------------init complete----------------------------
...
static const struct of_device_id altera_uart_match[] = {
	...
	{ .compatible = "altr,uart-1.0", },
	{ .compatible = "snps,dw-apb-uart-me" },
	{},
};
