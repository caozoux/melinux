...
cma_test_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < CMA_NUM; i++) {
		if (dma_virt[i]) {
			dma_free_coherent(cma_dev, (i + 1) * 8, dma_virt[i], dma_phys[i]);
...
cma_test_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < CMA_NUM; i++) {
		if (!dma_virt[i]) {
			dma_virt[i] = dma_alloc_coherent(cma_dev, (i + 1) * 1024, &dma_phys[i], GFP_KERNEL);
 
			if (dma_virt[i]) {
				void *p;
				/* touch every page in the allocated memory */
				for (p = dma_virt[i]; p <  dma_virt[i] + (i + 1) * 1024; p += PAGE_SIZE)
					*(u32 *)p = 0;
