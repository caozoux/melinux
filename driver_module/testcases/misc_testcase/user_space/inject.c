...
static void help(void)
{
	...
	printf("inject --hrtimer mesc  inject hrtimer run with mesc\n");
}
	...
	while (1) {
		...
		switch (option_index) {
			...
			case 4:
				...
				return ioctl(misc_fd, sizeof(struct ioctl_data), &data);
			case 5:
				data.inject_data.time = atoi(optarg);
			...
			case 7:
				...
				return ioctl(misc_fd, sizeof(struct ioctl_data), &data);
			case 8:
				data.cmdcode = IOCTL_INJECT_HRTIMER;
				data.inject_data.time = atoi(optarg);
				return ioctl(misc_fd, sizeof(struct ioctl_data), &data);
			default:
				break;
