...
int raidtree_unit_ioctl_func(unsigned int  cmd, unsigned long addr, struct ioctl_data *data)
{
	...
	switch (data->cmdcode) {
		case  IOCTL_USERAIDIXTREE_ADD:
			...
			//DEBUG("rx inert page:%d \n", data->raidix_data.index);
			if (copy_from_user(p, (char __user *) data->raidix_data.buf, data->raidix_data.buf_len))
				return -EINVAL;

			//ret = radix_tree_insert(&roottest, data->raidix_data.index, p);
			ret = radix_tree_insert(&rxtree_misc_data->root, data->raidix_data.index, p);
			if (ret)
		...
		case  IOCTL_USERAIDIXTREE_DEL:
			p = radix_tree_delete(&rxtree_misc_data->root, data->raidix_data.index);
			if (!p) {
		...
		case  IOCTL_USERAIDIXTREE_GET:
			p = radix_tree_lookup(&rxtree_misc_data->root, data->raidix_data.index);
			if (!p) {
			...
			copy_to_user((char __user *) data->raidix_data.buf, p, data->raidix_data.buf_len);
			break;
